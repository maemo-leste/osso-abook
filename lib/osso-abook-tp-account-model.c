...
#define PRIVATE(model) \
  ((OssoABookTpAccountModelPrivate *)osso_abook_tp_account_model_get_instance_private(((OssoABookTpAccountModel *)model)))
...
void
osso_abook_tp_account_model_set_required_capabilities(
    OssoABookTpAccountModel *model, OssoABookCapsFlags caps)
{
  g_return_if_fail(OSSO_ABOOK_IS_TP_ACCOUNT_MODEL(model));
